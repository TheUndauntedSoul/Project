...
public class PANE1 extends javax.swing.JFrame {
    ...
    //These are storage for Undertime and Late minutes
    ...
    public PANE1() {
        initComponents();
        updateAttendanceLabels(); //I need to do this so that lay 2 automatically displays the correct values...
 
//UI INITIALIZATION AND SCALING (1)
        
        // Prevents the window from being resized or maximized manually
        this.setResizable(false); 
        // This size is based dun sa dimensions to the extent of showing just the components and nothing else
        this.setMinimumSize(new java.awt.Dimension(1170, 720));
        
//TABLE MODEL SETUP (2)        
        
    //This is for constructing the custom table component using RejoruSan's table (More Customizable)
            ...
            
    {
        ...
        public boolean isCellEditable(int row, int col) {
            // 1. Prevents editing if the row is a REST DAY
            if (getValueAt(row, 1).toString().equals("REST DAY")) return false;

            // 2. Disables Leave column (Index 4) for Part-timers/Contractual
            if (col == 4) {
    ...
    rSTableMetro1.getModel().addTableModelListener(e -> {
        ...
        try {
            if (col == 3 || col == 4) {
                if (col == 3) { // If 'Absent' is checked
                    boolean isAbsent = (boolean) model.getValueAt(row, 3);
                    if (isAbsent) model.setValueAt(false, row, 4); // Uncheck Leave
                } else if (col == 4) { // If 'Leave' is checked
                    boolean isLeave = (boolean) model.getValueAt(row, 4);
                    if (isLeave) model.setValueAt(false, row, 3); // Uncheck Absent
                }
            }
...
// PLACEHOLDERS AND NAME FORMATTING LOGIC (3)    
    ...
    try {
        ...
        String type = jComboBox1.getSelectedItem().toString();
        double salaryInput = Double.parseDouble(jTextField4.getText().replace(",", ""));
        double loans = Double.parseDouble(jTextField3.getText());

        // 2. DYNAMIC WORKDAY COUNTING (WEEKENDS EXCLUDED)
        ...
        // 3. OBJECT-ORIENTED EMPLOYEE INITIALIZATION
        Employee emp;
        if (type.equalsIgnoreCase("Regular")) {
            emp = new RegularEmployee(id, name, salaryInput);
        } else if (type.equalsIgnoreCase("Probationary")) {
            emp = new ProbationaryEmployee(id, name, salaryInput);
        } else if (type.equalsIgnoreCase("Part-Timer")) {
            emp = new PartTimer(id, name, salaryInput);
        } else {
            emp = new ContractualEmployee(id, name, salaryInput);
        }
        ...
        // 5. EARNINGS (Using the Employee Object)
        double basePay = emp.calculateBasePay(actualWorkedHours); 

        // For Regulars, we still apply the dynamic deduction logic to handle lates fairly
        ...
        // 6. DEDUCTIONS (Fixed Basis for Part-Timers)
        // For Regulars, SSS/PH are based on their Monthly Rate. 
        // For Part-timers, they are based on their ACTUAL EARNINGS (Gross Pay).
        ...
        double pi = PayrollCalculator.getPagIBIGShare(); // Usually flat 100 or 200

        // Tax is ALWAYS based on actual taxable income (Gross - Deductions)
        ...
        jTextArea1.setText(String.format("""
            ...
            regularPay, otPay, grossPay, sss, ph, pi, tax, loans, (sss+ph+pi+tax+loans), netPay
        ));
    ...
    // Changes the label based on the option part-timer
...
// TABLE MANAGEMENT LOGIC (4)
    
    //This retrieves the data from the RStable in the correct formats (i.e the time strings transformed to decimal values) to be used in payslip calculations
    private void updateAttendanceLabels() {
        ...
        for (int i = 0; i < rSTableMetro1.getRowCount(); i++) {
            ...
            //Counter logic for RStable (Tracks absences and leaves)
            if (isAbs) { 
                absences++; 
            } else if (isLeave) { 
                leaves++; 
                ...
                //Counter logic for RStable (Tracks OT, UT, and Late)
        ...
        //These are the counters found on top of the RStable
    ...
    try {
        ...
        // Determine if leave should be allowed based on employee type
        boolean leaveStatus = canHaveLeave ? false : false; 
        model.addRow(new Object[]{dateLabel, "08:00", "17:00", false, leaveStatus});
        
            // This print statement 'reads' the variable and confirms logic in the console
            if (i == startDay) {
                System.out.println("Status: " + empType + " | Leave Eligible: " + canHaveLeave);
            }
        }
...
// HELPER FUNCTIONS (5)
    ...
    // Used dun sa table to see if the user entered a correct time format
    ...
    // Used in the RStable to split the time in hours and minutes
    ...
    // Basically does the same thing as the method above
...
} // This is the very last closing brace of the file YEHEY! HABA NG CODE NOH?!


//Hi Ma'am Anna! Hindi po kami natuwa sa pagbuo ng programang ito :D